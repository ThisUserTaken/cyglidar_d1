...
def generate_launch_description():
    ...
    lidar_node_3 = launch_ros.actions.Node(
        package = 'cyglidar_d1_ros2',
        executable = 'cyglidar_d1_publisher_3',
        output = 'screen',
        parameters=[
           {"port_number": "/dev/ttyUSB2"},
           {"baud_rate": LaunchConfiguration("baud_rate_mode")},
           {"frame_id": "laser_frame_3"},
           {"fixed_frame": "/map"},
    ...
    tf_node_3 = launch_ros.actions.Node(
        package = 'tf2_ros', executable = "static_transform_publisher", name="to_laserframe",
        arguments = ["0", "0", "0", "2.4", "0", "0", "map", "laser_frame_3"]
    )
    ...
    ld.add_action(lidar_node_3)
    ld.add_action(tf_node_3)
